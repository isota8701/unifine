...
class DimeNetPlusPlusWrap(DimeNetPlusPlus):
    def __init__(
        self,
        num_targets =cfg.GNN.hidden_dim ,
        hidden_channels=128,
        num_blocks=4,
        int_emb_size=64,
        basis_emb_size=8,
        ...
        otf_graph=False,
        cutoff=10.0,
        max_num_neighbors=cfg.MAX_ATOMS,
        envelope_exponent=5,
        num_before_skip=1,
        num_after_skip=2,
        num_output_layers=3,
    ...
    def forward(self, data):
        ...
        if self.otf_graph:
            edge_index, cell_offsets, neighbors = radius_graph_pbc_wrapper(
                data, self.cutoff, self.max_num_neighbors, data.num_atoms_t.device
            )
            data.edge_index_t = edge_index
            data.to_jimages_t = cell_offsets
            data.num_bonds_t = neighbors
        ...
        _, _, idx_i, idx_j, idx_k, idx_kj, idx_ji = self.triplets(
            edge_index, num_nodes=data.atom_types_t.size(0)
        )
        ...
        # Embedding block.
        x = self.emb(data.atom_types_t.long(), rbf, i, j)
